...
Tool result text is truncated to 50KB per result (same cap as `AgentspaceHook` already applies to broadcast events) before serialization. Tool call args are truncated to 2KB.
