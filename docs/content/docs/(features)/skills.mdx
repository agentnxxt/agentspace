...
How Agentspace extends agent capabilities with reusable skills from skills.sh.
...
Agentspace's skill system is fully compatible with [skills.sh](https://skills.sh) — the open agent skills ecosystem. Install any skill from the registry or create your own.
...
2. **Store it as a tool secret** so workers can authenticate:
   ...
   ```bash
   agentspace secrets set GH_TOKEN
...
3. **Install a GitHub skill** that teaches workers how to use the `gh` CLI:
   ...
   ```bash
   agentspace skill add anthropics/skills/github
   ```
...
agentspace skill add vercel-labs/agent-skills --instance

# Install to a specific agent
agentspace skill add remotion-dev/skills --agent my-agent
```

Install from a `.skill` file:

```bash
agentspace skill install path/to/skill.skill
```
...
```bash
mkdir -p ~/.agentspace/agents/my-agent/workspace/skills/my-skill
cd ~/.agentspace/agents/my-agent/workspace/skills/my-skill
```
...
|-------|-------------|---------|
| `vercel-labs/agent-skills` | React best practices, web design guidelines | `agentspace skill add vercel-labs/agent-skills` |
| `anthropics/skills/pdf` | PDF creation, editing, extraction | `agentspace skill add anthropics/skills/pdf` |
| `remotion-dev/skills` | Video generation with Remotion | `agentspace skill add remotion-dev/skills` |
| `supabase/agent-skills` | Postgres best practices for Supabase | `agentspace skill add supabase/agent-skills` |
...
```bash
ls -la ~/.agentspace/agents/my-agent/workspace/skills/my-skill/
...
```bash
rm -rf ~/.agentspace/agents/my-agent/workspace/skills/conflicting-skill
```
