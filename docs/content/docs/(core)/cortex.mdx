...
The cortex is the system's awareness of itself. Every other process in Agentspace is focused on a specific job — channels talk to users, branches think, workers execute, compactors manage context. None of them see the whole picture. The cortex does.
...
Agentspace also exposes a direct admin chat session with the cortex. This is intentionally self-referential and diagnostic-focused:
...
- `agentspace_docs` reads embedded Agentspace docs/changelog/AGENTS content directly from the binary.
...
If dispatch arrives while not ready, Agentspace does **not** block the channel or scheduler:
...
- `agentspace_dispatch_while_cold_count{agent_id,dispatch_type,reason}`
- `agentspace_warmup_recovery_latency_ms{agent_id,dispatch_type}`
