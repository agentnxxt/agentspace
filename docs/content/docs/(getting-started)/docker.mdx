---
title: Docker
description: Run Agentspace in a container with the unified Agentspace image.
---
...
One-click updates are intended for containers running Agentspace release tags. If you're running a custom/self-built image, update by rebuilding your image and recreating the container.

### Native / Source Builds

If Agentspace is installed from source (`cargo install --path .` or a local release build), updates are manual:

1. Pull latest source
2. Rebuild/reinstall the binary
3. Restart Agentspace
...
fly launch --image ghcr.io/agentnxxt/agentspace:latest
fly volumes create agentspace_data --size 5
