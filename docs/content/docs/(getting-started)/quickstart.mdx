---
title: Quickstart
description: Get Agentspace running locally in under 5 minutes.
---

# Quickstart

Get Agentspace running locally in under 5 minutes.
...
```bash
docker pull ghcr.io/agentnxxt/agentspace:latest
...
```bash
git clone https://github.com/agentnxxt/agentspace.git
cd agentspace
...
Agentspace needs at least one LLM provider key. You can either set an environment variable or create a config file.
...
This is enough. Agentspace will create a default `main` agent with sensible defaults and no messaging adapters. The web UI and HTTP API will be available on `http://localhost:19898`.

### Option B: Interactive onboarding

Just run `agentspace` with no config file and no API key env var set. It will walk you through provider selection, API key entry, agent naming, and optional Discord setup.

### Option C: Config file

Create `~/.agentspace/config.toml`:
...
agentspace start -f -d

# During development with cargo
cargo run -- start -f -d
```

On first launch, Agentspace automatically creates:

- `~/.agentspace/` — instance directory
- `~/.agentspace/agents/main/data/` — SQLite, LanceDB, and redb databases
- `~/.agentspace/agents/main/` — identity files (`SOUL.md`, `IDENTITY.md`, `ROLE.md`)
- `~/.agentspace/agents/main/workspace/` — working files and ingest directory (sandbox boundary for worker file tools)
...
agentspace restart -f -d  # restart in foreground with debug
```

Logs go to `~/.agentspace/agents/{id}/data/logs/` in daemon mode, or stderr in foreground mode.

## Identity files

Each agent has optional identity files in its root directory (`~/.agentspace/agents/{id}/`):
...
To run a dev instance from source alongside an installed production instance, use `AGENTSPACE_DIR` to give each its own data directory. Each instance gets separate databases, PID file, Unix socket, and logs — fully isolated.

### 1. Create a dev instance directory

```bash
mkdir -p ~/.agentspace/dev
```

### 2. Add a dev config with different ports

Create `~/.agentspace/dev/config.toml`:
...
```bash
# Production (installed binary, default ~/.agentspace)
agentspace start

# Dev (from source, separate data directory)
AGENTSPACE_DIR=~/.agentspace/dev cargo run -- start -f -d
```

The dev instance uses `~/.agentspace/dev/` for all data, and the production instance uses `~/.agentspace/` — they won't interfere with each other.

You can also set `AGENTSPACE_DIR` in a direnv `.envrc` so it applies automatically when you're in the project directory:

```bash
# .envrc in your agentspace source checkout
export AGENTSPACE_DIR="$HOME/.agentspace/dev"
```

Alternatively, use the `-c` flag to point at a specific config file (the instance directory is inferred as its parent):

```bash
cargo run -- -c ~/.agentspace/dev/config.toml start -f -d
```
...
No messaging adapters are required. Without them, Agentspace is accessible via the web UI and HTTP API.

## CLI flags reference

```
agentspace [OPTIONS] [COMMAND]
