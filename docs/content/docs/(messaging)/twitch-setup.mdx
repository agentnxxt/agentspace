---
title: Twitch Setup
description: Connect Agentspace to Twitch chat.
---

# Twitch Setup

Connect Agentspace to Twitch chat. Takes about 10 minutes.
...
3. Fill in the form:
   - **Name:** anything (e.g. "My Agentspace")
...
## Step 4: Add Credentials to Agentspace

Agentspace needs your **username**, **OAuth token**, **client ID**, **client secret**, and **refresh token**. With all five, Agentspace automatically refreshes expired tokens.

<Tabs items={["Agentspace UI", "TOML Config"]}>
<Tab value="Agentspace UI">

1. Open your Agentspace dashboard
...
Agentspace connects immediately — no restart needed.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `Login authentication failed` | Invalid or expired OAuth token | Re-run the token flow from Step 3. If you have `client_id`, `client_secret`, and `refresh_token` configured, Agentspace refreshes automatically — check that all three are set. |
