...
description: Connect Agentspace to Telegram.
---

# Telegram Setup

Connect Agentspace to Telegram. Takes about 2 minutes.
...
- **Name**: The display name (e.g. "My Agentspace")
- **Username**: Must end in `bot` (e.g. `my_agentspace_bot`)
...
## Step 3: Add Token to Agentspace
...
- **Groups**: Add the bot to the group, send a message, and check the Agentspace logs — the `conversation_id` will show as `telegram:<chat_id>`. Group IDs are negative numbers (e.g. `-100123456789`).
