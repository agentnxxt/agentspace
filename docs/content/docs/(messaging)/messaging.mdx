---
title: Messaging
description: How Agentspace connects to Discord, Slack, Telegram, Twitch, Email, and webhooks.
---

# Messaging

Agentspace connects to chat platforms so your agent can talk to people where they already are. You can connect multiple platforms at once — messages from all of them flow into the same agent system.
...
2. Agentspace opens a persistent connection to that platform
3. When someone sends a message, Agentspace receives it and decides whether to reply, skip, or delegate work
...
You can run multiple agents on the same Agentspace instance, each connected to different places. Each agent has its own memory, personality, and conversation history.
...
Each chat context maps to its own Agentspace conversation with isolated history:
