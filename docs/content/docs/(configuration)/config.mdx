...
Complete reference for `config.toml`. Agentspace looks for this file at `~/.agentspace/config.toml` (or `$AGENTSPACE_DIR/config.toml`). If no config file exists, it falls back to environment variables.

## File Location

```
~/.agentspace/config.toml          # default
$AGENTSPACE_DIR/config.toml        # env override
agentspace --config /path/to.toml  # CLI override
```
...
|----------|---------|-------------|
| `AGENTSPACE_DIR` | `~/.agentspace` | Instance root directory. All data, config, databases, and sockets live under this path. Set this to run multiple isolated instances (e.g. dev and prod) side by side. |
| `AGENTSPACE_DEPLOYMENT` | auto-detected | Deployment mode: `docker`, `hosted`, or native (auto-detected). Affects API bind address, agent limits, and update behavior. |
| `AGENTSPACE_MAX_AGENTS` | unlimited | Maximum number of agents (enforced in hosted mode only). |
| `AGENTSPACE_CRON_TIMEZONE` | server local | Default timezone for cron active-hours evaluation (IANA name). Overridden by `defaults.cron_timezone` or `agents.cron_timezone` in config. |
| `AGENTSPACE_USER_TIMEZONE` | inherits cron | Default timezone for channel/worker temporal context. Overridden by config equivalents. |
| `AGENTSPACE_CHANNEL_MODEL` | `anthropic/claude-sonnet-4-20250514` | Default channel model (env-only mode). |
| `AGENTSPACE_WORKER_MODEL` | `anthropic/claude-haiku-4.5-20250514` | Default worker model (env-only mode). |
...
base_url = "http://localhost:8080" # do not include /v1; Agentspace appends endpoint paths
...
id = "main"
default = true
workspace = "/custom/workspace/path"   # optional, defaults to ~/.agentspace/agents/{id}/workspace
...
LLM keys also have implicit env fallbacks — if no key is set in the TOML, Agentspace checks `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `KILO_API_KEY`, and `OPENCODE_GO_API_KEY` automatically.
...
If no `config.toml` exists, Agentspace runs from environment variables alone:

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
# optional model overrides
export AGENTSPACE_CHANNEL_MODEL="anthropic/claude-sonnet-4-20250514"
export AGENTSPACE_WORKER_MODEL="anthropic/claude-haiku-4.5-20250514"
agentspace
```
...
Most config values are hot-reloaded when their files change. Agentspace watches `config.toml`, identity files, and skill directories. Changes are debounced to 2 seconds and applied to all running channels, workers, and branches without restart.
...
- `~/.agentspace/config.toml`
- `~/.agentspace/skills/` (instance-level skills)
...
On file change, Agentspace re-reads the changed files and atomically swaps the new values into the live `RuntimeConfig` using `arc-swap`. All consumers (channels, branches, workers, compactors, cron jobs) read from `RuntimeConfig` on every use, so they pick up changes immediately.
...
> - Agentspace appends the endpoint path automatically:
...
If no prefix is provided (for example `claude-sonnet-4-20250514`), Agentspace defaults to the `anthropic` provider.
...
When branch/worker/cron dispatch happens before readiness is satisfied, Agentspace still dispatches, increments cold-dispatch metrics, and queues a forced warmup pass in the background.
...
| `workspace` | string | `~/.agentspace/agents/{id}/workspace` | Custom workspace path |
...
If a configured timezone is invalid, Agentspace logs a warning and falls back to server local time.

Channel/worker temporal context timezone precedence is:

1. `agents.user_timezone`
2. `defaults.user_timezone`
3. `AGENTSPACE_USER_TIMEZONE`
4. resolved cron timezone (from `agents.cron_timezone` / `defaults.cron_timezone` / `AGENTSPACE_CRON_TIMEZONE`)
