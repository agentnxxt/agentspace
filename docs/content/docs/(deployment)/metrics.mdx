---
title: Metrics
description: Prometheus-compatible metrics for monitoring Agentspace's LLM usage, costs, and agent activity.
---

# Metrics

Agentspace exposes Prometheus-compatible metrics for monitoring LLM costs, token usage, agent activity, and memory operations. All telemetry code is behind the `metrics` cargo feature flag — without it, every instrumentation block compiles out to nothing.
...
Add a `[metrics]` block to your `agentspace.toml`:
...
All metrics are prefixed with `agentspace_`.
...
| ------ | ---- | ------ | ----------- |
| `agentspace_llm_requests_total` | Counter | `agent_id`, `model`, `tier` | Total LLM completion requests |
| `agentspace_llm_request_duration_seconds` | Histogram | `agent_id`, `model`, `tier` | End-to-end LLM request duration |
| `agentspace_llm_tokens_total` | Counter | `agent_id`, `model`, `tier`, `direction` | Token counts (`direction`: input, output, cached_input) |
| `agentspace_llm_estimated_cost_dollars` | Counter | `agent_id`, `model`, `tier` | Estimated cost in USD |
...
| Metric | Type | Labels | Description |
| ------ | ---- | ------ | ----------- |
| `agentspace_tool_calls_total` | Counter | `agent_id`, `tool_name` | Total tool calls executed |
| `agentspace_tool_call_duration_seconds` | Histogram | — | Tool call execution duration |
...
| `agentspace_process_errors_total` | Counter | `agent_id`, `process_type`, `error_type` | Process errors by type |

### Memory Metrics

| Metric | Type | Labels | Description |
| ------ | ---- | ------ | ----------- |
| `agentspace_memory_reads_total` | Counter | — | Total memory recall operations |
| `agentspace_memory_writes_total` | Counter | — | Total memory save operations |
| `agentspace_memory_entry_count` | Gauge | `agent_id` | Total memory entries per agent |
| `agentspace_memory_updates_total` | Counter | `agent_id`, `operation` | Memory mutations (`operation`: save, update, delete, forget) |
