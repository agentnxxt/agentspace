---
title: Roadmap
description: Development timeline and future plans for Agentspace.
---
...
- **LLM** — `AgentspaceModel` implements Rig's `CompletionModel`, routes through `LlmManager` via HTTP with retries and fallback chains across 13 providers (Anthropic, OpenAI, OpenRouter, Kilo Gateway, Z.ai, Groq, Together, Fireworks, DeepSeek, xAI, Mistral, OpenCode Zen, OpenCode Go)
...
- **Hooks** — `AgentspaceHook` with tool call/result events, leak detection (11 patterns scanning both tool args and output); `CortexHook` for observation (trace-only)
...
- **Tools** — 20+ tools implement Rig's `Tool` trait with real logic (including task board tools, memory tools, `agentspace_docs`, and `config_inspect` alongside reply/branch/worker/browser/shell primitives)
...
- **Agent CLI** — `agentspace agents list/create/delete`, identity template bootstrapping
...
`AgentspaceModel.stream()` is implemented with provider SSE parsing. Remaining work is wiring stream-native Rig loops (`stream_prompt`) into runtime paths that still use non-streaming `.prompt(...)` calls (for example, cortex chat and channel loops).
...
- Tool nudging — inject "use your tools" in `AgentspaceHook.on_completion_response()` when LLM responds with text in early iterations
...
- `agentspace setup --quick --provider openrouter --key sk-...` — non-interactive fast path for scripted deploys
- `agentspace setup --channels-only` — add channels to an existing instance

## Decided Against

### Built-in Tunnel Support

Some self-hosted agent frameworks bundle Cloudflare/ngrok/Tailscale tunnel management into the binary. We intentionally don't — it's unnecessary for Agentspace's architecture.
...
For hosted instances, the platform proxy handles access over a private WireGuard mesh. For self-hosted instances, the embedded API and UI are accessible on your LAN. If you want remote access, Tailscale works out of the box — just install it on the host machine and access the Agentspace API via your tailnet. No integration needed, no config, no child process management.
...
Agentspace handles this through its control interface and the cortex. The embedded UI shows adapter status, agent health, and active connections in real-time. The cortex observes system-wide signals and surfaces issues proactively. Channel token validation happens at setup time through the bindings API — if something is misconfigured, you find out immediately when you add the binding, not by running `doctor` ten minutes later.
